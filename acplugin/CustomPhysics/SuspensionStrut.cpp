...
void SuspensionStrut_step(SuspensionStrut* pThis, float dt)
{
	...
	if (fHubDelta > pThis->bumpStopUp)
	{
		...
		vec3f vForce = vmul(makev(mxCarWorld.M21, mxCarWorld.M22, mxCarWorld.M23), -fForce);
		vec3f vPos = pThis->hub->getPosition(0.0f);
		pThis->addForceAtPos(vForce, vPos, false, false);

		vForce = makev(0, fForce, 0);
	...
	vec3f vDamperSpeed = vsub(vTyreStrutVel, vCarStrutVel);
	vDamperSpeed.x *= vDelta.x;
	vDamperSpeed.y *= vDelta.y;
	vDamperSpeed.z *= vDelta.z;

	float fDamperSpeed = vDamperSpeed.x + vDamperSpeed.y + vDamperSpeed.z;
