...
void BrakeSystem_step(BrakeSystem* pThis, float dt)
{
	...
	int v6 = (int)pThis->ebbMode - 1; // TODO
	if (v6)
	{
		if (v6 == 1)
	...
	else
	{
		...
		if (fLoadAWD != 0.0f)
		{
			float fSpeed = pThis->car->valueCache.speed.value; // getSpeed()
			if (fSpeed * 3.5999999f > 10.0f)
				bFlag = true;
		}
	...
	if (fFrontBias <= pThis->limitUp)
	{
		if (fFrontBias < pThis->limitDown)
			fFrontBias = pThis->limitDown;
	}
	else
	{
		fFrontBias = pThis->limitUp;
	}
...
void BrakeSystem_stepTemps(BrakeSystem* pThis, float dt)
{
	...
	for (int i = 0; i < 4; ++i)
	{
		...
		float fSpeed = pThis->car->valueCache.speed.value; // getSpeed()
		float fCool = (((fSpeed * 3.5999999f) * pDisc->coolSpeedFactor) + 1.0f) * pDisc->coolTransfer;
